...
class Admin extends CI_Controller {
    ...
    $data = [
        ...
        $this->load->model('Report_model', 'report'),
            ...
            'count_process' =>$this->db->select('*')->from('user_report')->like('status', 'On Process')->count_all_results(),
            'count_cancelled' =>$this->db->select('*')->from('user_report')->like('status', 'Cancelled')->count_all_results(),
            'count_settled' =>$this->db->select('*')->from('user_report')->like('status', 'Settled')->count_all_results(),
            'count_endorsed' =>$this->db->select('*')->from('user_report')->like('status', 'Endorsed')->count_all_results(),
            'count_dismiss' =>$this->db->select('*')->from('user_report')->like('status', 'Dismiss')->count_all_results(),
            'title' => 'Analytics',
            'user' => $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array(),
            'user_role' => $this->db->get('user_role')->num_rows(),
            'user_member' => $this->db->get_where('user', ['role_id' => 2])->num_rows(),
        ...
        $data['analyticsrecord'] = $this->report->getRecordResolvedandUnresolved();
        $data['analyticscomplaintrecord'] = $this->report->getRecordComplaintStatus();
    ...
    public function analytics_pdf()
    {
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
  
        $data['analytics'] = $this->Report_model->getTypeCount();
        $data['reports'] = $this->Report_model->getAll();      
        $data['analyticsrecord'] = $this->Report_model->getRecordResolvedandUnresolved();
        $data['analyticscomplaintrecord'] = $this->Report_model->getRecordComplaintStatus();
        $html = $this->load->view('templates/admin_header', $data);
        $html = $this->load->view('admin/analytics_pdf', [], true);
